    ...
    match solver.check_assumptions(&assumptions) {
        z3::SatResult::Sat => {
            ...
            for (var, value) in roots {
                ...
                // Use full-precision IBig rather than i64 to avoid spurious
                // collisions: values exceeding i64 range would all map to
                // i64::MAX, causing unrelated terms to be grouped together.
