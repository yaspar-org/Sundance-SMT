...
use yaspar_ir::ast::{ObjectAllocatorExt as _, Repr, StrAllocator, Term, TermAllocator, alg::QualifiedIdentifier};
...
pub fn nelson_oppen_clause(literal: i32, egraph: &mut Egraph) -> Option<Term> {
    ...
    match term.repr() {
        Eq(x, y) => {
            if x.get_sort(&mut egraph.cnfenv.context).to_string() == "Int" && y.get_sort(&mut egraph.cnfenv.context).to_string() == "Int" {
...
pub fn nelson_oppen_clause_pair(x: u64, y: u64, egraph: &mut Egraph) -> Option<Term> {
        ...
        let lt = QualifiedIdentifier::simple(egraph.cnfenv.context.allocate_symbol("<"));
        let lt_term = egraph.cnfenv.context.app(lt, vec![egraph.get_term(x), egraph.get_term(y)], Some(bool_sort.clone()));

        let gt = QualifiedIdentifier::simple(egraph.cnfenv.context.allocate_symbol(">"));
        let gt_term = egraph.cnfenv.context.app(gt, vec![egraph.get_term(x), egraph.get_term(y)], Some(bool_sort));

        let eq_term = egraph.cnfenv.context.eq(egraph.get_term(x), egraph.get_term(y));
