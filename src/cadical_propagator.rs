...
impl<'a> ExternalPropagator for CustomExternalPropagator<'a> {
    ...
    fn cb_check_found_model(&mut self, model: &[i32]) -> bool {
        ...
        if let Some(mut pending) = self.pending.take() {
            eprintln!("SERVE_PENDING: level={} remaining_sk={} remaining_inst={}", self.decision_level, pending.deferred_skolemizations.len(), pending.deferred_instantiations.len());
        ...
        eprintln!("FRESH_COMPUTE: level={} sk={} inst={}", self.decision_level, pending.deferred_skolemizations.len(), pending.deferred_instantiations.len());
