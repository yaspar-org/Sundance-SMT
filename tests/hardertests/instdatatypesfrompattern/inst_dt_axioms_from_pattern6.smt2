...
(declare-const l lib)
(declare-fun po () lib)
(assert (forall ((k P)) (! (not (b! $ Y (ol l) k (ol po))) :pattern ((b! $ Y (ol l) k (ol l))))))
(assert (= (st (P la) (P (%l (ly (li o))))) t))
(assert (= td (v $ Y $ TY t)))
(assert (= la (%P (P la))))
(assert (= B (%% la (P la))))
(assert (= (% B) (b! $ Y (ol (i (%Po Pol))) (ol (ib $ Y Pol)) (ol (i (%Po Pol))))))
(declare-const k P)
(assert (not (a (%% (%%l $ Y (v $ Y $ TY (st (P la) (P (%l (ly (li o)))))) $ Y $ TY (P la) $ Y $ TY (Poly (b (Po (oly (lib!delegation_map_t.AbstractDelegationMap./AbstractDelegationMap (%Pol (y lib!delegation_map_v.DelegationMap./DelegationMap)))))))) k) TY)))
(assert (= (ite (s $ Y (v $ Y $ TY (st (P la) (P (%l (ly (li o)))))) k) (vst $ Y $ Y k k) (vs $ Y $ TY k k)) (%% (%%l $ Y (v $ Y $ TY (st (P la) (P (%l (ly (li o)))))) $ Y $ TY (P la) $ Y $ TY (Poly (b (Po (oly (lib!delegation_map_t.AbstractDelegationMap./AbstractDelegationMap (%Pol (y lib!delegation_map_v.DelegationMap./DelegationMap)))))))) k)))
(assert (a (vs $ Y $ TY k k) TY))
(assert (= (s $ Y td k) (% (%% (%P (P la)) (P la)))))
(check-sat)
