...
(declare-fun apply (Poly Poly) Bool)
...
; instantiations
(assert (= (contains (intersection s1! s2!) a) (apply (intersection s1! s2!) a)))
(assert (= (contains (intersection s1! s3!) a) (apply (intersection s1! s3!) a)))
(assert (= (contains s1! a) (apply s1! a)))
(assert (= (contains (union s2! s3!) a) (apply (union s2! s3!) a)))
(assert (= (contains (intersection s1! (union s2! s3!)) a) (apply (intersection s1! (union s2! s3!)) a)))
(assert (= (contains (union (intersection s1! s2!) (intersection s1! s3!)) a) (apply (union (intersection s1! s2!) (intersection s1! s3!)) a)))


; I think we are missing one of these
(assert (forall ((%%hole%%2 Poly)(%%hole%%5 Poly) (x$ Poly)) (! (= (apply (union %%hole%%2 %%hole%%5) x$) (or (contains %%hole%%2 x$) (contains %%hole%%5 x$))) :pattern ((apply (union %%hole%%2 %%hole%%5) x$)))))
; instantiations
...
(assert (forall ((%%hole%%2 Poly)(%%hole%%5 Poly) (x$ Poly)) (! (= (apply (intersection %%hole%%2 %%hole%%5) x$) (and (contains %%hole%%2 x$) (contains %%hole%%5 x$))) :pattern ((apply (intersection %%hole%%2 %%hole%%5) x$)))))
; instantiations
...
;(assert (= (apply (union s2! s3!) a) (or (contains s2! a) (contains s3! a))))
;(assert (= (apply (union (intersection s1! s2!) (intersection s1! s3!)) a) (or (contains (intersection s1! s2!) a) (contains (intersection s1! s3!) a))))
;(assert (= (apply (intersection s1! s2!) a) (and (contains s1! a) (contains s2! a))))
;(assert (= (apply (intersection s1! s3!) a) (and (contains s1! a) (contains s3! a))))
;(assert (= (apply (intersection s1! (union s2! s3!)) a) (and (contains s1! a) (contains (union s2! s3!) a))))
