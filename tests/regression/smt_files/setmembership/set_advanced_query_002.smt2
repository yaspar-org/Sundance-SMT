...
;; Function-Specs set_advanced::empty_in_power_set
(declare-fun ens%set_advanced!empty_in_power_set. (Dcr Type Poly) Bool)
(declare-fun %%lambda%%16 (Poly) %%Function%%)
(assert
 (forall ((%%hole%%0 Poly) (x$ Poly)) (!
   (= (%%apply%%0 (%%lambda%%16 %%hole%%0) x$) %%hole%%0)
   :pattern ((%%apply%%0 (%%lambda%%16 %%hole%%0) x$))
)))
(assert
 (forall ((T&. Dcr) (T& Type) (s! Poly)) (!
   (= (ens%set_advanced!empty_in_power_set. T&. T& s!) (set_advanced!is_member.? $ (TYPE%vstd!set.Set.
      T&. T&
     ) (vstd!set.impl&%0.new.? T&. T& (Poly%fun%1. (mk_fun (%%lambda%%16 (B false)))))
     (set_advanced!power_set.? T&. T& s!)
   ))
   :pattern ((ens%set_advanced!empty_in_power_set. T&. T& s!))
   :qid internal_ens__set_advanced!empty_in_power_set._definition
   :skolemid skolem_internal_ens__set_advanced!empty_in_power_set._definition
)))

;; Function-Def set_advanced::empty_in_power_set
;; set_advanced.rs:45:7: 45:42 (#0)
(get-info :all-statistics)
 ...
 (assert
  (not (=>
    %%location_label%%0
    (set_advanced!is_member.? $ (TYPE%vstd!set.Set. T&. T&) (vstd!set.impl&%0.new.? T&.
      T& (Poly%fun%1. (mk_fun (%%lambda%%16 (B false))))
     ) (set_advanced!power_set.? T&. T& s!)
 ))))
 (get-info :version)
