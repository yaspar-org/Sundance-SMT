...
(declare-const nats@ vstd!set.Set<nat.>.)
(assert fuel_defaults)
(assert (<= 0 bound!))
(assert (forall ((i$ Poly)) (! (=> (has_type i$ NAT) (= (and (<= 0 (%I i$)) (< (%I i$) bound!)) (vstd!set.impl&%0.contains.? $ NAT (Poly%vstd!set.Set<nat.>. s!) i$))) :pattern ((vstd!set.impl&%0.contains.? $ NAT (Poly%vstd!set.Set<nat.>. s!) i$)) )))
(declare-const %%location_label%%0 Bool)
(declare-const %%location_label%%1 Bool)
(declare-const %%location_label%%2 Bool)
(declare-const %%location_label%%3 Bool)
(assert (not (=> (= nats@ (lib!pigeonhole.set_nat_range.? (I 0) (I bound!))) (and (=> %%location_label%%0 (req%lib!pigeonhole.lemma_nat_range. 0 bound!)) (=> (ens%lib!pigeonhole.lemma_nat_range. 0 bound!) (=> (= tmp%1 (ext_eq false (TYPE%vstd!set.Set. $ NAT) (Poly%vstd!set.Set<nat.>. s!) (Poly%vstd!set.Set<nat.>. nats@))) (and (=> %%location_label%%1 tmp%1) (=> tmp%1 (and (=> %%location_label%%2 (vstd!set.impl&%0.finite.? $ NAT (Poly%vstd!set.Set<nat.>. s!))) (=> %%location_label%%3 (= (vstd!set.impl&%0.len.? $ NAT (Poly%vstd!set.Set<nat.>. s!)) bound!)))))))))))
(check-sat)
(exit)
