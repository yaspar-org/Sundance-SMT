...
(assert (has_type (Poly%fun%1. f!) (TYPE%fun%1. A&. A& $ INT)))
(assert (> (vstd!seq.Seq.len.? A&. A& s!) 0))
(declare-fun %%lambda%%2 () %%Function%%)
(assert (forall ((x$ Poly) (y$ Poly)) (! (= (%%apply%%1 %%lambda%%2 x$ y$) (I (Add (%I x$) (%I y$)))) :pattern ((%%apply%%1 %%lambda%%2 x$ y$)))))
(declare-const %%location_label%%0 Bool)
(declare-const %%location_label%%1 Bool)
(declare-const %%location_label%%2 Bool)
(assert (not (=> (= g@ (mk_fun %%lambda%%2)) (=> (= tmp%1 (mk_fun (%%lambda%%1 f!))) (=> (= tmp%2 (mk_fun (%%lambda%%0 f! g@))) (and (=> %%location_label%%0 (req%vstd!function.fun_ext_2. $ INT A&. A& $ INT tmp%1 tmp%2)) (=> (ens%vstd!function.fun_ext_2. $ INT A&. A& $ INT tmp%1 tmp%2) (and (=> %%location_label%%1 (req%lib!verus_extra.seq_lib_v.lemma_seq_fold_left_merge_right_assoc. A&. A& $ INT s! (I low!) f! g@)) (=> (ens%lib!verus_extra.seq_lib_v.lemma_seq_fold_left_merge_right_assoc. A&. A& $ INT s! (I low!) f! g@) (=> %%location_label%%2 (= (Add (%I (vstd!seq_lib.impl&%0.fold_left.? A&. A& $ INT (vstd!seq.Seq.subrange.? A&. A& s! (I 0) (I (Sub (vstd!seq.Seq.len.? A&. A& s!) 1))) (I low!) (Poly%fun%2. (mk_fun (%%lambda%%1 f!))))) (%I (%%apply%%0 f! (vstd!seq.Seq.index.? A&. A& s! (I (Sub (vstd!seq.Seq.len.? A&. A& s!) 1)))))) (%I (vstd!seq_lib.impl&%0.fold_left.? A&. A& $ INT s! (I low!) (Poly%fun%2. (mk_fun (%%lambda%%1 f!))))))))))))))))
(check-sat)
(exit)
