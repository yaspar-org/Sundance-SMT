...
(declare-const head@ Poly)
(declare-const tail@ Poly)
(declare-const p@ Poly)
(declare-const s1$1@ Poly)
(declare-const s2@ Poly)
(declare-const emp@ Poly)
(declare-const len@ Int)
(declare-const i@ Int)
(declare-const s1@ Poly)
(declare-const last@ Poly)
...
(assert (has_type (Poly%fun%2. g!) (TYPE%fun%2. B&. B& B&. B& B&. B&)))
(assert (> (vstd!seq.Seq.len.? A&. A& s!) 0))
(assert (forall ((x$ Poly) (y$ Poly) (z$ Poly)) (! (=> (and (has_type x$ B&) (has_type y$ B&) (has_type z$ B&)) (= (%%apply%%1 g! (%%apply%%1 g! x$ y$) z$) (%%apply%%1 g! x$ (%%apply%%1 g! y$ z$)))) :pattern ((%%apply%%1 g! (%%apply%%1 g! x$ y$) z$)) )))
...
(assert (not (=> (= decrease%init0 (vstd!seq.Seq.len.? A&. A& s!)) (=> (= emp@ (vstd!seq.Seq.empty.? B&. B&)) (=> (= len@ (vstd!seq.Seq.len.? A&. A& s!)) (=> (= i@ (Sub len@ 1)) (=> (= s1@ (vstd!seq.Seq.subrange.? A&. A& s! (I 0) (I (Sub len@ 1)))) (=> (= last@ (vstd!seq.Seq.index.? A&. A& s! (I (Sub len@ 1)))) (=> (= accf@ (mk_fun (%%lambda%%0 f! g!))) (=> (= start@ (vstd!seq_lib.impl&%0.fold_left.? A&. A& B&. B& s1@ init! (Poly%fun%2. accf@))) (=> (= all@ (vstd!seq_lib.impl&%0.fold_left.? A&. A& B&. B& s! init! (Poly%fun%2. accf@))) (or (and (=> (= (vstd!seq.Seq.len.? A&. A& s1@) 0) (=> (= tmp%1 (= (vstd!seq.Seq.len.? A&. A& s!) 1)) (and (=> %%location_label%%0 tmp%1) (=> tmp%1 (=> (fuel_bool fuel%vstd!seq_lib.impl&%0.fold_left.) (=> (exists ((fuel% Fuel)) (= fuel_nat%vstd!seq_lib.impl&%0.fold_left. (succ fuel%))) (=> (fuel_bool fuel%vstd!seq_lib.impl&%0.fold_left.) (=> (exists ((fuel% Fuel)) (= fuel_nat%vstd!seq_lib.impl&%0.fold_left. (succ fuel%))) %%switch_label%%0)))))))) (=> (not (= (vstd!seq.Seq.len.? A&. A& s1@) 0)) (=> (fuel_bool fuel%vstd!seq_lib.impl&%0.fold_left.) (=> (exists ((fuel% Fuel)) (= fuel_nat%vstd!seq_lib.impl&%0.fold_left. (succ fuel%))) (=> (= head@ (vstd!seq.Seq.index.? A&. A& s! (I 0))) (=> (= tail@ (vstd!seq.Seq.subrange.? A&. A& s! (I 1) (I len@))) (=> (= p@ (%%apply%%1 accf@ init! (vstd!seq.Seq.index.? A&. A& s! (I 0)))) (=> (= s1$1@ (vstd!seq.Seq.subrange.? A&. A& tail@ (I 0) (I (Sub len@ 2)))) (=> (= s2@ (vstd!seq.Seq.subrange.? A&. A& s1@ (I 1) (I (Sub len@ 1)))) (and (=> (= tmp%2 (= (vstd!seq.Seq.len.? A&. A& s1$1@) (vstd!seq.Seq.len.? A&. A& s2@))) (and (=> %%location_label%%1 tmp%2) (=> tmp%2 (and (=> (has_type idx@ INT) (=> (and (<= 0 (%I idx@)) (< (%I idx@) (vstd!seq.Seq.len.? A&. A& s1$1@))) (=> %%location_label%%2 (= (vstd!seq.Seq.index.? A&. A& s1$1@ idx@) (vstd!seq.Seq.index.? A&. A& s2@ idx@))))) (=> (forall ((idx$ Poly)) (! (=> (has_type idx$ INT) (=> (and (<= 0 (%I idx$)) (< (%I idx$) (vstd!seq.Seq.len.? A&. A& s1$1@))) (= (vstd!seq.Seq.index.? A&. A& s1$1@ idx$) (vstd!seq.Seq.index.? A&. A& s2@ idx$)))) :pattern ((vstd!seq.Seq.index.? A&. A& s1$1@ idx$)) :pattern ((vstd!seq.Seq.index.? A&. A& s2@ idx$)) )) (=> (= tmp%3 (ext_eq false (TYPE%vstd!seq.Seq. A&. A&) s1$1@ s2@)) (and (=> %%location_label%%3 tmp%3) (=> tmp%3 (=> %%location_label%%4 (= s1$1@ s2@)))))))))) (=> (= s1$1@ s2@) (and (=> %%location_label%%5 (check_decrease_int (let ((s!$0 tail@) (init!$1 p@) (f!$2 f!) (g!$3 g!)) (vstd!seq.Seq.len.? A&. A& s!$0)) decrease%init0 false)) (and (=> %%location_label%%6 (req%lib!verus_extra.seq_lib_v.lemma_seq_fold_left_merge_right_assoc. A&. A& B&. B& tail@ p@ f! g!)) (=> (ens%lib!verus_extra.seq_lib_v.lemma_seq_fold_left_merge_right_assoc. A&. A& B&. B& tail@ p@ f! g!) %%switch_label%%0)))))))))))))) (and (not %%switch_label%%0) (=> %%location_label%%7 (= (%%apply%%1 g! (vstd!seq_lib.impl&%0.fold_left.? A&. A& B&. B& (vstd!seq.Seq.subrange.? A&. A& s! (I 0) (I (Sub (vstd!seq.Seq.len.? A&. A& s!) 1))) init! (Poly%fun%2. (mk_fun (%%lambda%%0 f! g!)))) (%%apply%%0 f! (vstd!seq.Seq.index.? A&. A& s! (I (Sub (vstd!seq.Seq.len.? A&. A& s!) 1))))) (vstd!seq_lib.impl&%0.fold_left.? A&. A& B&. B& s! init! (Poly%fun%2. (mk_fun (%%lambda%%0 f! g!)))))))))))))))))))
(check-sat)
(exit)
