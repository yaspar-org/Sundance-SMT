...
(assert (forall ((A&. Dcr) (A& Type) (self! Poly)) (! (= (req%vstd!seq_lib.impl&%0.drop_first. A&. A& self!) (=> %%global_location_label%%3 (>= (vstd!seq.Seq.len.? A&. A& self!) 1))) :pattern ((req%vstd!seq_lib.impl&%0.drop_first. A&. A& self!)) )))
...
(assert (forall ((seq! Poly)) (! (= (req%lib!util_v.nat_seq_max. seq!) (=> %%global_location_label%%4 (< 0 (vstd!seq.Seq.len.? $ NAT seq!)))) :pattern ((req%lib!util_v.nat_seq_max. seq!)) )))
...
(assert (has_type seq! (TYPE%vstd!seq.Seq. $ NAT)))
(declare-const %%switch_label%%0 Bool)
(declare-const %%switch_label%%1 Bool)
(declare-const %%location_label%%0 Bool)
(assert (not (=> (= decrease%init0 (vstd!seq.Seq.len.? $ NAT seq!)) (or (and (=> (= (vstd!seq.Seq.len.? $ NAT seq!) 1) (=> (= tmp%3 (vstd!seq.Seq.index.? $ NAT seq! (I 0))) %%switch_label%%0)) (=> (not (= (vstd!seq.Seq.len.? $ NAT seq!) 1)) (or (and (=> (= (vstd!seq.Seq.len.? $ NAT seq!) 0) (=> (= tmp%2 0) %%switch_label%%1)) (=> (not (= (vstd!seq.Seq.len.? $ NAT seq!) 0)) (=> (= tmp%1 (vstd!seq_lib.impl&%0.drop_first.? $ NAT seq!)) (and (=> %%location_label%%0 (check_decrease_int (let ((seq!$0 (%Poly%vstd!seq.Seq<nat.>. tmp%1))) (vstd!seq.Seq.len.? $ NAT (Poly%vstd!seq.Seq<nat.>. seq!$0))) decrease%init0 false)) (=> (<= 0 later_max@) (=> (= tmp%2 (ite (>= (%I (vstd!seq.Seq.index.? $ NAT seq! (I 0))) later_max@) (%I (vstd!seq.Seq.index.? $ NAT seq! (I 0))) later_max@)) %%switch_label%%1)))))) (and (not %%switch_label%%1) (=> (= tmp%3 (I tmp%2)) %%switch_label%%0))))) (not %%switch_label%%0)))))
(check-sat)
(exit)
