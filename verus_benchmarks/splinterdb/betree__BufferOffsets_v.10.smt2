...
(declare-fun %%lambda%%1 (Dcr Type Poly Int Dcr Type Poly Dcr Type Poly Int Dcr Type Poly Int Int Int) %%Function%%)
(assert (forall ((%%hole%%0 Dcr) (%%hole%%1 Type) (%%hole%%2 Poly) (%%hole%%3 Int) (%%hole%%4 Dcr) (%%hole%%5 Type) (%%hole%%6 Poly) (%%hole%%7 Dcr) (%%hole%%8 Type) (%%hole%%9 Poly) (%%hole%%10 Int) (%%hole%%11 Dcr) (%%hole%%12 Type) (%%hole%%13 Poly) (%%hole%%14 Int) (%%hole%%15 Int) (%%hole%%16 Int) (i$ Poly)) (! (= (%%apply%%0 (%%lambda%%1 %%hole%%0 %%hole%%1 %%hole%%2 %%hole%%3 %%hole%%4 %%hole%%5 %%hole%%6 %%hole%%7 %%hole%%8 %%hole%%9 %%hole%%10 %%hole%%11 %%hole%%12 %%hole%%13 %%hole%%14 %%hole%%15 %%hole%%16) i$) (I (ite (<= (%I (vstd!seq.Seq.index.? %%hole%%0 %%hole%%1 %%hole%%2 i$)) %%hole%%3) (%I (vstd!seq.Seq.index.? %%hole%%4 %%hole%%5 %%hole%%6 i$)) (ite (< (%I (vstd!seq.Seq.index.? %%hole%%7 %%hole%%8 %%hole%%9 i$)) %%hole%%10) %%hole%%16 (nClip (Add (Sub (%I (vstd!seq.Seq.index.? %%hole%%11 %%hole%%12 %%hole%%13 i$)) %%hole%%14) %%hole%%15)))))) :pattern ((%%apply%%0 (%%lambda%%1 %%hole%%0 %%hole%%1 %%hole%%2 %%hole%%3 %%hole%%4 %%hole%%5 %%hole%%6 %%hole%%7 %%hole%%8 %%hole%%9 %%hole%%10 %%hole%%11 %%hole%%12 %%hole%%13 %%hole%%14 %%hole%%15 %%hole%%16) i$)))))
...
(assert (fuel_bool_default fuel%lib!betree.BufferOffsets_v.empty_bufer_offsets.))
(assert (=> (fuel_bool fuel%lib!betree.BufferOffsets_v.empty_bufer_offsets.) (forall ((no%param Poly)) (! (= (lib!betree.BufferOffsets_v.empty_bufer_offsets.? no%param) (lib!betree.BufferOffsets_v.BufferOffsets./BufferOffsets (%Poly%vstd!seq.Seq<nat.>. (vstd!seq.Seq.empty.? $ NAT)))) :pattern ((lib!betree.BufferOffsets_v.empty_bufer_offsets.? no%param)) ))))
...
(declare-const self! lib!betree.BufferOffsets_v.BufferOffsets.)
(declare-const target! Int)
(declare-const limit! Int)
(assert fuel_defaults)
(assert (<= 0 target!))
(assert (<= 0 limit!))
(assert (lib!betree.BufferOffsets_v.impl&%0.all_gte.? (Poly%lib!betree.BufferOffsets_v.BufferOffsets. self!) (I target!)))
(assert (lib!betree.BufferOffsets_v.impl&%0.all_lte.? (Poly%lib!betree.BufferOffsets_v.BufferOffsets. self!) (I limit!)))
(assert (>= limit! target!))
(declare-const %%location_label%%0 Bool)
(assert (not (=> %%location_label%%0 (lib!betree.BufferOffsets_v.impl&%0.all_lte.? (Poly%lib!betree.BufferOffsets_v.BufferOffsets. (lib!betree.BufferOffsets_v.impl&%0.shift_left.? (Poly%lib!betree.BufferOffsets_v.BufferOffsets. self!) (I target!))) (I (nClip (Sub limit! target!)))))))
(check-sat)
(exit)
